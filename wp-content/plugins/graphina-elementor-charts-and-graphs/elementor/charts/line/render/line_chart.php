...
<div class="<?php echo $settings['iq_line_chart_card_show'] === 'yes' ? 'chart-card' : ''; ?>">
    <div class="">
        <?php if ($settings['iq_line_is_card_heading_show'] && $settings['iq_line_chart_card_show']) { ?>
            <h4 class="heading graphina-chart-heading" style="text-align: <?php echo $settings['iq_line_card_title_align'];?>;  color: <?php echo strval($settings['iq_line_card_title_font_color']);?>;"><?php echo html_entity_decode($title); ?></h4>
        <?php }
        if ($settings['iq_line_is_card_desc_show'] && $settings['iq_line_chart_card_show']) { ?>
            <p class="sub-heading graphina-chart-sub-heading" style="text-align: <?php echo $settings['iq_line_card_subtitle_align'];?>;  color: <?php echo strval($settings['iq_line_card_subtitle_font_color']);?>;"><?php echo html_entity_decode($description); ?></p>
        <?php } ?>
    </div>
    <?php if (!empty($settings['iq_line_dynamic_change_chart_type']) && $settings['iq_line_dynamic_change_chart_type'] == 'yes') { ?>
        <div class="graphina_dynamic_change_type">
            <select id="graphina-select-chart-type"
                    onchange="updateChartType('line',this,'<?php esc_attr_e($this->get_id()); ?>');">
...
<!--                <option value="scatter">Scatter</option>-->
    ...
    <?php }
    graphina_filter_common($this,$settings,$type);
    ?>
    <div class="<?php echo $settings['iq_line_chart_border_show'] === 'yes' ? 'chart-box' : ''; ?>">
        ...
        <div style="display: none;height: <?php echo $settings['iq_' . $this->get_chart_type() . '_chart_height'];?>" class="chart-texture line-chart-<?php esc_attr_e($this->get_id()); ?>-loader" >
